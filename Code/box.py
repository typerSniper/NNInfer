...
def get_underapprox_box(activation_pattern, weights, biases, min_values, max_values, additional_constraints, epsilon):
	...
	prob += lpSum([(pulpInputs[i][1]-pulpInputs[i][0]) for i in range(num_inputs)]), "Total Range is Maximized"
	for layer_id,layer in enumerate(activation_pattern):
		...
		for neuron_id,neuron in enumerate(layer):
			...
			if neuron==0:
				prob += lpSum(w_t_hi) <= -1 * b, constraint_name_hi
				prob += lpSum(w_t_lo) <= -1 * b, constraint_name_lo
			elif neuron==1:
				prob += lpSum(w_t_hi) > -1 * b, constraint_name_hi
				prob += lpSum(w_t_lo) > -1 * b, constraint_name_lo
	for constraint_id, constraint in enumerate(additional_constraints):
		...
		prob += lpSum(w_t_hi) > rhs, constraint_name_hi
		prob += lpSum(w_t_lo) > rhs, constraint_name_lo
	print (prob)
	status = prob.solve()   
